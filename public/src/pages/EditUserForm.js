...
class EditUserForm extends Page {
  constructor(path, userEmail) {
    super(path);
    this.userEmail = userEmail;
  }

  async renderPage() {
    this.editForm = await this.createForm();
    this.editForm.addEventListener('onViewChange', Router.goBack);
    this.$view.append(this.editForm.$view);
  }

  async createForm() {
    const user = await DataService.getUserData(this.userEmail);
